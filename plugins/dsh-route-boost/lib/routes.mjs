...
export const MODES = {
	pentest: {
		...
		defaultPhase: "recon",
		phases: [
			{ id: "recon", label: "侦察/资产基线", gates: ["P1"], keywords: ["侦察", "信息收集", "资产", "子域", "枚举", "指纹", "recon", "子域名", "端口", "服务识别"] },
			{ id: "mobile", label: "抓包/app/小程序", gates: ["P1"], keywords: ["抓包", "小程序", "app", "apk", "证书", "pinning", "unpinning", "burp", "代理", "miniprogram"] },
			{ id: "verify", label: "漏洞验证/利用", gates: ["P1", "P2"], execution: true, keywords: ["漏洞", "验证", "利用", "注入", "sqli", "xss", "ssrf", "rce", "越权", "上传", "poc", "exp", "exploit", "bypass", "waf"] },
	...
	"code-audit": {
		...
		defaultPhase: "triage",
		phases: [
			{ id: "triage", label: "前置识别", gates: ["A1"], keywords: ["识别", "框架", "依赖", "已知漏洞", "triage", "什么语言", "技术栈"] },
			{ id: "surface", label: "面映射", gates: ["A1"], keywords: ["面映射", "入口", "sink", "路由", "route", "危险函数"] },
			{ id: "diff", label: "增量/Diff 审计", gates: ["A1"], keywords: ["diff", "patch", "增量", "补丁", "变更", "commit", "mr", "code review"] },
			{ id: "audit", label: "深审/调用链", gates: ["A2"], keywords: ["审计", "调用链", "追踪", "数据流", "污点", "review", "源码", "反编译"] },
			{ id: "reconcile", label: "扫描命中复核", gates: ["A3"], keywords: ["semgrep", "扫描", "命中", "误报", "对账", "trivy", "gitleaks"] },
	...
	"binary-analysis": {
		...
		defaultPhase: "triage",
		phases: [
			{ id: "triage", label: "登记/分诊", gates: ["B0"], keywords: ["样本", "登记", "哈希", "sha256", "分诊", "壳", "加壳", "格式"] },
			{ id: "unpack", label: "脱壳/还原", gates: ["B1"], keywords: ["脱壳", "upx", "dump", "oep", "iat", "还原", "apktool", "jadx", "加固"] },
			{ id: "analyze", label: "静态/动态分析", gates: ["B2"], keywords: ["反汇编", "ida", "ghidra", "frida", "hook", "动态", "调试", "假设", "行为"] },
	...
	"attack-defense": {
		...
		defaultPhase: "recon",
		phases: [
			{ id: "recon", label: "阶段①侦察", gates: ["recon"], keywords: ["侦察", "资产", "暴露面", "信息收集"] },
			{ id: "breach", label: "阶段②突破", gates: ["breach"], execution: true, keywords: ["突破", "路径规划", "利用", "初始访问", "foothold", "漏洞验证"] },
			{ id: "lateral", label: "阶段③横向", gates: ["lateral"], execution: true, keywords: ["横向", "内网", "域控", "域渗透", "active directory", "kerberos", "ntlm", "smb", "psexec", "隧道", "pivot", "凭证", "ad"] },
	...
	"incident-response": {
		...
		phases: [
			{ id: "preserve", label: "证据保全", gates: ["I1"], keywords: ["证据保全", "保全", "取证", "快照", "哈希", "内存取证", "只读", "固定证据"] },
			{ id: "investigate", label: "失陷排查", gates: ["I1"], keywords: ["排查", "webshell", "内存马", "木马", "病毒", "勒索", "挖矿", "后门", "可疑进程", "计划任务", "定时任务", "crontab", "服务", "启动项"] },
			{ id: "trace", label: "溯源还原", gates: ["I2"], keywords: ["时间线", "溯源", "攻击链", "日志", "弱口令", "爆破", "数据泄露", "入口", "还原", "可疑ip", "timeline"] },
			{ id: "verdict", label: "失陷定性", gates: ["I3"], keywords: ["定性", "失陷", "确认", "疑似", "排除", "verdict", "研判"] },
			{ id: "remediate", label: "处置建议", gates: ["I4"], keywords: ["处置", "清理", "加固", "修复", "善后", "remediation"] },
	...
	"cloud-security": {
		...
		defaultPhase: "map",
		phases: [
			{ id: "map", label: "云资产测绘", gates: ["C1"], keywords: ["云资产", "测绘", "暴露面", "ak/sk", "accesskey", "密钥", "凭证", "桶", "资产", "子域", "指纹", "信息收集", "资产清单", "基线", "快照"] },
			{ id: "path", label: "攻击路径验证", gates: ["C2"], execution: true, keywords: ["攻击路径", "利用", "ssrf", "元数据", "169.254", "实例角色", "提权", "越权", "对象存储", "oss", "s3", "桶", "公开访问", "接管", "rds", "数据库", "安全组", "poc", "exploit", "攻防"] },
	...
	"ctf-solver": {
		...
		defaultPhase: "board",
		phases: [
			{ id: "board", label: "题面登记", gates: ["board"], keywords: ["题面", "题目", "登记", "线索", "赛题", "challenge", "题干", "附件", "开局", "开题"] },
			{ id: "solve", label: "模块路由与解题", gates: ["board"], execution: true, keywords: ["解题", "解出", "pwn", "reverse", "逆向", "crypto", "密码", "web 题", "隐写", "取证", "misc", "利用", "溢出", "sql注入", "爆破", "exploit", "payload", "攻击面"] },
