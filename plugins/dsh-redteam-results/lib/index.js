...
function apply(ctx) {
	...
	ctx.tools.register(defineTool({
		...
		parameters: {
			...
			type: { type: "string", description: "类型（按模式词表）" },
			description: { type: "string", description: "描述（影响与成因）" },
			poc: { type: "string", description: "测试过程/复现 EXP" },
			chain: { type: "string", description: "链（语义按模式）" },
			chainTracer: { type: "string", description: "追踪员链（审计）" },
			chainVerdict: { type: "string", description: "双链结论（审计）" },
			snippetEntry: { type: "string", description: "入口片段（审计）" },
			snippetSink: { type: "string", description: "sink 片段（审计）" },
			cwe: { type: "string", description: "CWE 编号" },
			patch: { type: "string", description: "修复 diff（审计）" },
			...
			evidence: { type: "string", description: "证据引用（evidence-index）" },
