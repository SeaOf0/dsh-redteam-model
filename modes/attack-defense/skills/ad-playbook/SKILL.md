---
name: ad-playbook
description: 攻防评估模式作战手册：Agentic Red Teaming 定位（AEV 持续验证）、阶段编排（侦察/突破/横向/持久化/报告+复测闭环）、内网攻防作战流程（常见端口侦察+蜜罐甄别/服务线弱口令+锁定策略探测/被动凭据线（同广播域投毒中继）/SSH·Windows·SMB·数据库落点线/框架 EXP/DevOps 与邮件·OA·VPN 高价值线/隔离突破 C 段→B 段/域攻纵深分级（BloodHound→Roasting→ADCS→委派→DCSync）/堡垒机与安防设备优先/穷尽终止+机器可读黑板与操作痕迹台账）、防御验证（detection gap）、AI 应用红队（Garak/PyRIT/Promptfoo 工具矩阵）、验证与评分、MITRE ATT&CK 映射、证据收集规范与报告模板。
---
