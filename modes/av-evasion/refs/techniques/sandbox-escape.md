...
## 9. Process Injection Detection with Volatility and Sysmon

### Volatility Malfind — Detect Injected Code

```bash
# Find injected code segments in memory dump
vol.py -f memory.dmp windows.malfind.Malfind
# Looks for: PAGE_EXECUTE_READWRITE sections with no backing file

# HollowFind — detect process hollowing
vol.py -f memory.dmp windows.hollowfind.HollowFind
# Detects: PE headers that don't match the on-disk binary
```

### Injection Technique Classification

| Technique | API Sequence | Detection Trace |
|---|---|---|
| DLL Injection | OpenProcess -> VirtualAllocEx -> WriteProcessMemory -> CreateRemoteThread | Remote thread with LoadLibrary start address |
| Process Hollowing | CreateProcess(suspended) -> NtUnmapViewOfSection -> VirtualAllocEx -> WriteProcessMemory -> SetThreadContext -> ResumeThread | Mismatched PE headers in memory vs disk |
| APC Injection | OpenProcess -> VirtualAllocEx -> WriteProcessMemory -> QueueUserAPC | APC queued to thread in remote process |
| Thread Hijacking | OpenThread -> SuspendThread -> GetThreadContext -> SetThreadContext -> ResumeThread | Thread instruction pointer modified |
| Reflective DLL Injection | Custom loader (no LoadLibrary call) | Memory region with RWX permissions, no file backing |
| Process Doppelganging | CreateTransaction -> CreateFileTransacted -> WriteFile -> CreateSectionFromFile -> CreateProcess | NTFS transaction-based process creation |
| AtomBombing | GlobalAddAtom -> GlobalGetAtomName -> QueueUserAPC | Atom table abuse for code injection |

### Sysmon Event Analysis

```python
# Parse Sysmon events for injection indicators
# EID 8: CreateRemoteThread — classic injection
# EID 10: ProcessAccess with suspicious access masks (e.g., PROCESS_VM_WRITE)
# EID 1: Process creation with suspicious parent-child relationships
```
...
| T1055.002 | PE Injection | Windows |
...
| T1055.009 | /proc/pid/mem | Linux |
| T1055.011 | Extra Window Memory | Windows |
...
### Sigma Rule — CreateRemoteThread Detection
...
## 10. Malware Sandbox Evasion Techniques (T1497)

### Timing Checks

```c
// Check for sleep acceleration in sandboxes
DWORD t1 = GetTickCount();
Sleep(5000);
DWORD t2 = GetTickCount();
if ((t2 - t1) < 4500) exit(0);  // Sleep was accelerated — sandbox detected
```
...
| MAC address prefixes | `00:0C:29` (VMware), `08:00:27` (VirtualBox), `00:50:56` (VMware) |
| Process names | `vmtoolsd.exe`, `vboxservice.exe`, `xenservice.exe` |
| Driver files | `C:\Windows\System32\drivers\vmmouse.sys`, `vboxmouse.sys` |

### User Interaction Checks

- Mouse movement tracking (sandboxes rarely move mouse)
...
- Disk size check (sandbox VMs typically small disks)
- CPU count (sandbox VMs often 1-2 cores)
- Memory capacity check
- Number of installed applications
- System uptime (freshly booted VMs = suspicious)
